...
import { ENERPACK_EMPLOYEE_MASTER } from './enerpackEmployeeMaster';

export const ATTENDANCE_EMPLOYEES: EmployeeProfile[] = ENERPACK_EMPLOYEE_MASTER.map(e => ({
  id: e.id,
  name: e.name,
  ...
  shiftHours: 10,
  email: e.email || `${e.name.toLowerCase().replace(/\s+/g, '.')}@enerpack.in`,
  phone: e.mobile || '+91 98450 12345',
}));
