...
import { 
  ...
  Clock, ShieldAlert, Plus, Edit2, Download, UserCheck, Eye, ExternalLink
} from 'lucide-react';
...
============================================================`;
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs overflow-y-auto animate-in fade-in duration-200">
                    ...
                    return (
                      <div key={d.id} className="p-4 bg-white border border-slate-200 rounded-xl shadow-xs flex flex-col justify-between">
                        ...
                        <div className="mt-3 pt-3 border-t border-slate-100 flex items-center justify-between">
                          <span className="text-[11px] text-slate-400 truncate max-w-[160px]">{d.fileName || 'document.pdf'}</span>
                          <div className="flex items-center gap-1.5">
                            <button 
                              onClick={() => setViewingDoc(d)}
                              className="px-2 py-1 bg-slate-100 text-slate-700 hover:bg-slate-200 rounded text-xs font-semibold flex items-center gap-1 transition-colors"
                            >
                              <Eye className="w-3.5 h-3.5 text-blue-600" /> View
                            </button>
                            <button 
                              onClick={() => handleDownloadDoc(d)}
                              className="px-2 py-1 text-xs font-semibold text-blue-600 hover:text-blue-700 hover:bg-blue-50 rounded flex items-center gap-1 transition-colors"
                            >
      ...
      {/* Embedded Document Viewer for Vehicle Detail Modal */}
      {viewingDoc && (
        <div className="fixed inset-0 z-60 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs animate-in fade-in duration-150">
          <div className="bg-white rounded-2xl shadow-2xl border border-slate-200 w-full max-w-2xl overflow-hidden my-6 flex flex-col max-h-[85vh]">
            <div className="p-4 border-b border-slate-200 flex items-center justify-between bg-slate-50 shrink-0">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <h3 className="font-bold text-slate-900 text-sm">{viewingDoc.documentType}</h3>
                  <p className="text-xs text-slate-500 font-mono">{viewingDoc.vehicleNumber} • Doc #{viewingDoc.documentNumber}</p>
                </div>
              </div>
              <button 
            ...
            <div className="p-5 overflow-y-auto space-y-4 flex-1 custom-scrollbar">
              <div className="bg-slate-50 rounded-xl border border-slate-200 p-4 min-h-[260px] flex items-center justify-center">
                {viewingDoc.fileData ? (
                  viewingDoc.fileType?.startsWith('image/') || viewingDoc.fileName?.match(/\.(png|jpg|jpeg|webp)$/i) ? (
                    <img 
                      ...
                      className="max-h-[45vh] mx-auto rounded-lg shadow-sm object-contain border border-slate-200 bg-white" 
                    />
                  ) : (
                    <iframe 
                      src={viewingDoc.fileData} 
                  ...
                  <div className="w-full max-w-md bg-white rounded-xl border border-slate-200 shadow-2xs p-5 space-y-3">
                    <div className="flex items-center justify-between border-b border-slate-100 pb-2">
                      <span className="text-xs font-bold text-slate-900 uppercase">Enerpack Vault Copy</span>
                      <span className="px-2 py-0.5 rounded text-[10px] font-bold bg-emerald-50 text-emerald-700">Verified</span>
                    </div>
                    <div className="grid grid-cols-2 gap-2 text-xs">
                      <div>
                        <span className="text-[10px] text-slate-400 block">Document</span>
                        <span className="font-semibold text-slate-800">{viewingDoc.documentType}</span>
                      </div>
                      <div>
                        <span className="text-[10px] text-slate-400 block">Doc Number</span>
                        <span className="font-mono text-slate-800 font-medium">{viewingDoc.documentNumber}</span>
                      </div>
                      <div>
                        <span className="text-[10px] text-slate-400 block">Authority</span>
                        <span className="text-slate-700">{viewingDoc.issuingAuthority || 'Govt Transport'}</span>
                      </div>
                      <div>
                        <span className="text-[10px] text-slate-400 block">Valid Until</span>
                        <span className="font-semibold text-slate-900">{viewingDoc.expiryDate}</span>
                      </div>
                    </div>
                    {viewingDoc.notes && (
                      <p className="text-xs text-slate-500 bg-slate-50 p-2 rounded border border-slate-100">{viewingDoc.notes}</p>
                    )}
                  </div>
                )}
              </div>
